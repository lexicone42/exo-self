...
pub fn get_usage_ratio(paths: &ExoPaths, transcript_path: &str, estimated_max_chars: u64) -> (f64, &'static str) {
    // Priority 1: statusline-written .context-window.json
    if let Ok(data) = std::fs::read_to_string(&paths.context_window) {
        if let Ok(ctx) = serde_json::from_str::<ContextWindow>(&data) {
            if now() - ctx.updated_at < 120.0 {
