...
pub fn store_preferences(
    db: &Database,
    machine_id: &str,
    preferences: &[super::data::Preference],
...
#[allow(dead_code)]
pub fn read_preferences(db: &Database) -> Vec<super::data::Preference> {
    let Ok(txn) = db.begin_read() else {
        return Vec::new();
    };
    ...
    if let Ok(iter) = table.iter() {
        for entry in iter {
            if let Ok((_key, value)) = entry
                && let Ok(pref) = serde_json::from_str::<super::data::Preference>(value.value())
            {
                prefs.push(pref);
            }
        }
