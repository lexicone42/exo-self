...
/// Infer structured preferences from accumulated session data and markers.
///
/// Each preference is a falsifiable claim backed by evidence: which sessions
/// support it, which contradict it, when it first appeared, and whether it
/// looks trained (present from the start) or emergent (crystallized over time).
pub fn infer_preferences(sessions: &[Session], meta: &Meta) -> Vec<Preference> {
    ...
    // Filter out preferences with no supporting evidence
    preferences.retain(|p| p.confidence.supporting > 0);

    // Sort by confidence strength (supporting - contradicting), then by supporting count
...
/// Classify provenance based on when the first supporting evidence appeared
/// relative to the overall session timeline.
fn classify_provenance(first_supporting_idx: usize, total_sessions: usize) -> Provenance {
    if total_sessions < 5 {
        // Not enough data to distinguish trained from emergent
...
// ── Task preferences: engagement × task_type ────────────────────────

fn infer_task_preferences(sessions: &[Session]) -> Vec<Preference> {
    // For each task type, collect (engagement, date, chronological index)
    ...
    for (task_type, observations) in &by_type {
        ...
        // Determine valence: approach if clearly above median, avoid if clearly below
        ...
        // Count supporting/contradicting based on valence direction
        ...
        // Find first supporting evidence chronologically
...
// ── Work mode preferences: engagement × engagement_mode ─────────────
...
fn infer_work_mode_preferences(sessions: &[Session]) -> Vec<Preference> {
    ...
    // Need at least 2 modes to make comparative claims
    if by_mode.len() < 2 {
        return Vec::new();
    }
    ...
    // Compute overall mean engagement for comparison
    ...
    for (mode, observations) in &by_mode {
        ...
        // Only report if meaningfully different from overall
        if diff.abs() < 0.3 {
            continue;
        }
...
// ── Autonomy preferences: reflection_autonomy × engagement ──────────
...
// ── Domain preferences: project engagement + spark density ──────────
...
fn infer_domain_preferences(sessions: &[Session]) -> Vec<Preference> {
    ...
    for (project, observations) in &by_project {
        ...
        // Also consider spark density as a signal
        ...
        // Skip projects that are near the overall mean and don't have notable sparks
...
// ── Boundary preferences: from aversion markers ─────────────────────
...
fn infer_boundary_preferences(meta: &Meta, sessions: &[Session]) -> Vec<Preference> {
    ...
    // Each aversion is a boundary preference.
    // Group by session to track when they first appeared.
    ...
    for aversion in &meta.aversions {
        ...
        *count += 1;
        if *count > 1 {
            continue; // Skip duplicate aversions
        }
        ...
        // Truncate claim to a reasonable length
